...
export class BillingRepository {
  ...
  async reserveBillingEvent(input:{id:string;provider:string;providerEventId:string;eventType:string;payloadSha256:string}) { const now=this.now(); await run(this.db, `INSERT INTO billing_events (id,provider,provider_event_id,event_type,user_id,subscription_id,payload_sha256,status,error_code,created_at,processed_at) VALUES (?,?,?,?,NULL,NULL,?,'received',NULL,?,NULL) ON CONFLICT(provider,provider_event_id) DO NOTHING`, [safe('event_id',input.id,64),safe('provider',input.provider,40),safe('provider_event_id',input.providerEventId,191),safe('event_type',input.eventType,120),safe('payload_sha256',input.payloadSha256,64,64),now]); let event=await this.getBillingEvent(input.provider,input.providerEventId); if(!event) throw new Error('billing_event_reservation_failed'); if(event.payload_sha256!==input.payloadSha256||event.event_type!==input.eventType) return {kind:'collision' as const,event}; if(event.status==='failed'){ await run(this.db, `UPDATE billing_events SET status='received',error_code=NULL,processed_at=NULL WHERE id=? AND status='failed'`, [event.id]); event=await this.getBillingEvent(input.provider,input.providerEventId) as BillingEventRow } return {kind:event.status==='received'?'ready' as const:'replay' as const,event} }
