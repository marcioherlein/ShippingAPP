...
async function applyProviderResource(repo: BillingRepository, local: BillingSubscriptionRow, provider: BillingProviderSubscription, expectedEmail: string | null) {
  ...
  if(!updated) throw new Error('billing_reconcile_write_failed'); return updated
}
async function fetchAuthorizedPaymentPreapprovalId(env:Env,id:string,fetcher:typeof fetch) {
  ...
  let response:Response; try { response=await fetcher(`https://api.mercadopago.com/authorized_payments/${encodeURIComponent(id)}`,{headers:{authorization:`Bearer ${token}`,accept:'application/json'},signal:AbortSignal.timeout(12000)}) } catch { throw new BillingProviderError('billing_provider_unavailable') }
  ...
  const value=typeof body?.preapproval_id==='string'?body.preapproval_id.trim():''; if(!value||value.length>191) throw new BillingProviderError('billing_provider_invalid_response'); return value
}

async function checkout(request:Request,env:Env,userId:string,repo:BillingRepository,provider:BillingProvider) {
  ...
  const requestedPlan=typeof body?.planCode==='string'?body.planCode.trim():''; const configuration=resolveBillingPlanConfiguration(env,requestedPlan)
  if(!configuration) return json({error:'Plan no disponible.',code:'billing_plan_unavailable'},400)
  ...
  const subscriptionId=await stableId('sub',userId,operationKey)
  const reserved=await repo.reserveCheckout({subscriptionId,userId,planId:plan.id,provider:'mercadopago',providerPlanId:configuration.providerPlanId,idempotencyKey:operationKey})
  ...
  if(reserved.kind==='existing') {
    if(reserved.subscription.checkout_state==='created'&&reserved.subscription.checkout_url) return json({status:reserved.subscription.status,subscription:publicSubscription(reserved.subscription),checkoutUrl:reserved.subscription.checkout_url,replayed:true})
    if(reserved.subscription.provider_subscription_id) {
      try { const current=await provider.getSubscription(reserved.subscription.provider_subscription_id); const updated=await applyProviderResource(repo,reserved.subscription,current,payerEmail); const persisted=await repo.markCheckoutCreated({userId,subscriptionId,providerSubscriptionId:current.id,providerCustomerId:current.payerId,providerStatus:current.status,providerVersion:current.version,internalStatus:updated.status,checkoutUrl:current.checkoutUrl}); return json({status:persisted?.status??updated.status,subscription:publicSubscription(persisted??updated),checkoutUrl:current.checkoutUrl,replayed:true}) } catch { return json({status:'processing',subscription:publicSubscription(reserved.subscription),replayed:true},202) }
    }
  ...
  try {
    ...
    const current=await provider.getSubscription(created.id); const invalid=validateProviderResource(reserved.subscription,current,payerEmail)
    if(invalid) { await repo.markCheckoutFailed(userId,subscriptionId,invalid); return json({error:'Mercado Pago devolvió una suscripción inconsistente.',code:'billing_provider_identity_mismatch'},502) }
    ...
    return json({status:updated.status,subscription:publicSubscription(updated),checkoutUrl:updated.checkout_url,replayed:false},201)
...
async function cancelOwned(request:Request,userId:string,repo:BillingRepository,provider:BillingProvider) {
  ...
  const user=await repo.getUser(userId); try { await provider.cancelSubscription(local.provider_subscription_id); const current=await provider.getSubscription(local.provider_subscription_id); const updated=await applyProviderResource(repo,local,current,validEmail(user?.email)); return json({status:updated.status,subscription:publicSubscription(updated)}) } catch(error) { return json({error:'No pudimos cancelar la suscripción.',code:error instanceof BillingProviderError?error.code:'billing_cancel_failed'},502) }
}
async function webhook(request:Request,env:Env,repo:BillingRepository,provider:BillingProvider,deps:Dependencies) {
  ...
  try {
    ...
    const current=await provider.getSubscription(providerSubscriptionId); let local=await repo.getSubscriptionByProviderId('mercadopago',current.id)
...
export async function handleBilling(request:Request,env:Env,deps:Dependencies={}):Promise<Response> {
  ...
  if(url.pathname==='/api/billing/checkout'&&request.method==='POST') return checkout(request,env,userId,repo,provider)
