...
/**
 * Alibaba's search-engine-facing country/showroom URLs often index products
 * under a compact technical discriminator rather than the full marketing title
 * (for example `100m-watch`). Derive those routes without guessing commerce
 * facts; the downstream parser still requires the exact selected product_id.
 */
export function highSignalAlibabaPublicQueries(target: URL, hints: { name?: string | null; category?: string | null } = {}) {
  ...
  for (const discriminator of discriminatorTokens(tokens).slice(0, 3)) {
    // 42.5MM is typically model/case size; 100m waterproof, 65w power, 128gb
    // storage, etc. are useful public-index discriminators. Exact product_id
    // matching still protects against mixing a neighbouring SKU.
...
export function highSignalAlibabaPublicUrls(target: URL, hints: { name?: string | null; category?: string | null } = {}) {
  ...
  for (const query of highSignalAlibabaPublicQueries(target, hints)) {
    ...
    if (!value) continue
    // Prefer countrysearch first: in live Alibaba this surface is materially
    // more likely to expose commerce cards than guessed full-title showroom URLs.
...
export async function corroborateAlibabaHighSignalRoutes(
  ...
  if (!urls.length) {
    return { status: 'unavailable', source: 'Alibaba public listing corroboration', facts: null, pagesAttempted: 0, warnings: ['No high-signal Alibaba public routes could be derived from the product identity.'] }
  }
