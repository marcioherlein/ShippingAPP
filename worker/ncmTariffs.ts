...
export function deriveNcmHierarchy(code: string): NcmHierarchy {
  ...
  return {
    section: sectionForChapter(chapter),
    chapter,
...
export function validateTariffIndex(index: unknown): asserts index is NcmTariffIndex {
  ...
  if (!raw || raw.meta?.tariffSchema !== 1 || !Array.isArray(raw.records) || !Array.isArray(raw.conflicts)) {
    throw new Error('NCM tariff index failed schema checks')
  }
  if (raw.records.length < 10_000 || raw.meta.recordCount !== raw.records.length || raw.meta.conflictCount !== raw.conflicts.length) {
  ...
  for (const row of raw.records) {
    if (!Array.isArray(row) || row.length !== 7 || !NCM_PATTERN.test(row[0])) {
      throw new Error('NCM tariff index contains malformed record')
    }
    if (seen.has(row[0])) throw new Error(`NCM tariff index contains duplicate ${row[0]}`)
    seen.add(row[0])
    if (!row.slice(1).every(finiteNonNegative)) throw new Error(`NCM tariff index contains invalid rates for ${row[0]}`)
  }
...
export function lookupNcmTariff(index: NcmTariffIndex, code: string): NcmTariffLookup {
  ...
  const row = index.records.find((item) => item[0] === code)
  if (!row) {
    return {
  ...
  return {
    ...
    rates: {
      aecPct: row[1], statisticsPct: row[2], ivaPct: row[3], ivaAdditionalPct: row[4],
    },
    sourceReference: {
      profitsPct: row[5], iibbPct: row[6], automaticApplicationAllowed: false,
    },
    warnings: [
      'Ganancias e IIBB se conservan sólo como referencia de la planilla fuente; no se aplican automáticamente porque dependen del importador, jurisdicción y operación.',
    ],
    source: index.meta.source, sourceDate: index.meta.sourceDate,
  }
}
...
export function resetNcmTariffCacheForTests() {
  cachedIndex = null
}
