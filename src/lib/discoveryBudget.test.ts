...
function item(overrides: Partial<ProductDiscoveryItem> = {}): ProductDiscoveryItem {
  return {
...
describe('budget-aware discovery', () => {
  ...
  it('keeps missing price or MOQ as unknown instead of inventing feasibility', () => {
  ...
  it('puts known possible candidates before unknown candidates while preserving tie order', () => {
    const result = applyDiscoverySearchContext([
  ...
  it('leaves candidates active when no onboarding context is supplied', () => {
    const result = applyDiscoverySearchContext([item({ title: 'A' }), item({ title: 'B', unitPriceUsd: null })], {})
